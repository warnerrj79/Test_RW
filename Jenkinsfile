  ...
  stage('Build') {
    ...
    dir('chkRobTest') {
      checkout scm
       ...
       C:\\PS\\Build.ps1 -SourceControlDBFolder pwd+"\\chkRobTest"  -TempDBName (New-DatabaseConnection -ServerInstance ROB\\BUILD -Database RW_TEST -Username Jenkins -Password xxx) -NugetPackageID pckTestRWx -NugetPackageVersion "${env:VersionNumber}" -NugetPackageOutputDir C:\\nuget\\Test_RWp'''
  }
